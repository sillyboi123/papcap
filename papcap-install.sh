...
echo 'echo "THE PID OF THIS SCRIPT TO KILL LATER IS BELOW, THE FIRST LINE SECOND SEQUENCE OF NUMBERS BEFORE "/usr/local/sbin/rp-pppoe-server.sh" STARTING FROM THE LEFT TO THAT PATH, USE kill PIDNUMBER TO FINISH" ' >> /usr/local/sbin/rp-pppoe-server.sh
