...
echo 'echo "THE PID OF THIS SCRIPT TO KILL LATER IS BELOW, THE FIRST LINE SECOND SEQUENCE OF NUMBERS BEFORE "sudo /usr/local/sbin/rp-pppoe-server.sh", USE kill PIDNUMBER TO FINISH" ' >> /usr/local/sbin/rp-pppoe-server.sh
